...
class TableStructureRecognizer(Recognizer):
    ...
    def __call__(self, images, thr=0.2):
        ...
        for tbl in tbls:
            ...
            col_left = [b["x0"] for b in lts if b["label"] == "table column"]
            col_right = [b["x1"] for b in lts if b["label"] == "table column"]
            if col_left:
                left = min(left, np.min(col_left))
            if col_right:
                right = max(right, np.max(col_right))
            for b in lts:
    ...
    @staticmethod
    def __join_cell_lines(arr, line_gap=8.0):
        # Gom các mảnh text trong cùng 1 ô theo dòng (dựa vào toạ độ "top"),
        # giữ được xuống dòng thật trong ô thay vì gộp phẳng thành 1 dòng.
        items = sorted(arr, key=lambda c: (c.get("top", 0), c.get("x0", 0)))
        lines, cur_top, cur_line = [], items[0].get("top", 0), []
        for c in items:
            top = c.get("top", 0)
            if top - cur_top <= line_gap:
                cur_line.append(c)
            else:
        ...
        lines.append(cur_line)
        return "<br>".join(
            " ".join(c["text"] for c in sorted(ln, key=lambda c: c.get("x0", 0)))
            for ln in lines
        )
    ...
    def __markdown_table(cap, hdset, tbl):
        ...
        for cell in tbl[header_row]:
            if not cell:
                ...
                txt = TableStructureRecognizer.__join_cell_lines(cell)
                headers.append(txt)
        md += "| " + " | ".join(headers) + " |\n"
        md += "| " + " | ".join(["---"] * len(headers)) + " |\n"
        # Compose rows
